...
const appointmentsUrl = "http://localhost:4003/appo";
...
const header = ["Patient ID & Doctor ID", "Problem", "Status"];
...
export default function Appointments() {
  ...
  let ranges = 1;
  appointments.length % other["pagination-range"] == 0
    ? (ranges = appointments.length / other["pagination-range"])
    : (ranges = appointments.length / other["pagination-range"] + 1);

  useEffect(() => {
    getAxios(appointmentsUrl).then((res) => {
  ...
  return (
    <section className="bg-gray-50 dark:bg-gray-900 py-3 sm:py-5">
        ...
        <div className="relative overflow-hidden bg-white shadow-md dark:bg-gray-800 sm:rounded-lg">
            ...
            <form id="table-search" className="max-w-md mx-3">
              ...
              <div
                ...
                onClick={() => {
                  ...
                  const rowFilter = searchInString(appointments, valueToSearch);
                  setappointments(rowFilter);
                }}
              >
          ...
          <div className="flex flex-col px-4 py-3 space-y-3 lg:flex-row lg:items-center lg:justify-between lg:space-y-0 lg:space-x-4">
            <div className="flex items-center flex-1 space-x-4">
              <h5>
                <span className="text-gray-500">{"All appointments: "}</span>
                <span className="dark:text-white">{appointments.length} </span>
              </h5>
            </div>
            <div className="flex flex-col flex-shrink-0 space-y-3 md:flex-row md:items-center lg:justify-end md:space-y-0 md:space-x-3">
              <button
                ...
                onClick={() =>
                  setopenModalAddappointment(!openModalAddappointment)
                }
              >
                <svg
                  ...
                  {header.map((head) => (
                    <th scope="col" className="px-6 py-3" key={head}>
                      <div
                        ...
                        onClick={() => {
                          setappointments(orderElement(appointments, ""));
                        }}
                      >
                        {head}
                        <button
                        ...
                        <td className="w-4 px-4 py-3">
                          <div className="flex items-center">
                            <button
                              onClick={() => {
                                deleteAxios(
                                  appointmentsUrl + `/${appointment.id}`
                                ).then((res) => {
                                  console.log("elment deleted: ", res.data);
                                  setappointments(
                                    appointments.filter((e) => {
                                      return e.id != appointment.id;
                                    })
                                  );
                                });
                              }}
              ...
              <ul className="inline-flex items-stretch -space-x-px">
                ...
                <li>
                  <button
                    ...
                    onClick={() => {
                      let val = indexPagination + other["pagination-range"];
                      if (val <= appointments.length) setindexPagination(val);
                    }}
                  >
      ...
      {/* Add new element */}
      <Modal
        show={openModalAddappointment}
        size="md"
        onClose={() => setopenModalAddappointment(false)}
        popup
      >
        <Modal.Header />
        <Modal.Body>
            ...
            <form method="post">
              <div id="data" className="flex flex-col gap-3">
                ...
                <div className="max-w-md">
                  <div className="mb-2 block">
                    <Label htmlFor="file" value="appointment photo" />
                  </div>
                  <FileInput
                    id="imagefileUpload"
                    name="img-url"
              ...
              >
                <Button
                  ...
                  type="submit"
                  // onClick={() => setopenModalAddappointment(false)}
                >
